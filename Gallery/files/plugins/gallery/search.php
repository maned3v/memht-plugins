...
class SearchPlugin extends searchModel {
	public function InPlugin($query) {
		...
		$plugname = isset($plugmatch['gallery']) ? $plugmatch['gallery'] : "gallery" ;
		...
		//...by query
		$query = $Db->_e($query);
		$where[] = "(a.title LIKE '%".$query."%' OR a.name LIKE '%".$query."%' OR a.description LIKE '%".$query."%')";
		//...by author
		...
		$searchres = false;
		if ($result = $Db->GetList("SELECT a.*,c.name AS cname, c.title AS ctitle, u.name AS author_name, s.name AS sname, s.title AS stitle
									FROM #__gallery AS a JOIN #__gallery_categories AS c JOIN #__gallery_sections AS s JOIN #__user AS u
									ON a.category=c.id AND s.id=c.section AND a.author=u.uid
									{$where}
			...
			foreach ($result as $row) {
				...
				$searchres[] = array('title'	=>$title,
									 ...
									 'subtitle'	=>_t("ADDED_IN_X_BY_Y_ON_Z",
												  $ctitle,
